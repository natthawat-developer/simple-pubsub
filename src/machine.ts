...
export class MachineSaleSubscriber implements ISubscriber {
  ...
  handle(event: MachineSaleEvent): void {
      ...
      if (machine) {
          ...
          console.log(`Machine ${machine.id} sold ${event.getSoldQuantity()} units, stock left: ${machine.stockLevel}`);
...
export class MachineRefillSubscriber implements ISubscriber {
  ...
  handle(event: MachineRefillEvent): void {
      ...
      if (machine) {
          ...
          console.log(`Machine ${machine.id} refilled with ${event.getRefillQuantity()} units, new stock: ${machine.stockLevel}`);
          if (machine.stockLevel >= 3 && machine.lowStockWarningSent) {
